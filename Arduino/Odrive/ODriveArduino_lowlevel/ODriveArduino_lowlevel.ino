...
//  }
  ...
  Serial.println("Send the character 'E' To calibrate the encoder");
  Serial.println("Send the character 'C' close loop ");
...
//        float pos_m0 = 0.0f * cos(ph);
//        float pos_m1 = 0.0f * sin(ph);
...
//  }
 // Sinusoidal test move
    if (c == 's') {
      Serial.println("Executing test move");
      for (float ph = 0 ; ph < 0.5; ph += 0.03) {
        odrive.SetPosition(1, ph);
        Serial.println(ph);
        delay(5);
      }
    }
    ...
    if (c == 'p') {
        long encoder = odrive.GetPosition(1);
    ...
    if (c == 't') {
        float actualP = odrive.GetPosition(1);
        for (float ph = 0 ; ph < 0.5; ph += 0.03) {
          odrive.SetPosition(1, actualP + ph);
          Serial.println(odrive.GetPosition(1));
          delay(50);
         }    
        actualP = odrive.GetPosition(1);
        for (float ph = 0 ; ph < 0.48; ph += 0.03) {
          odrive.SetPosition(1, actualP - ph);
          Serial.println(odrive.GetPosition(1));
          delay(50);
         }            
